...
to setup
  ...
  let possible-strategies ["random" "always cooperate" "always defect" "grim trigger" "tit for tat"]

  ask patches [
    set strategy one-of Fight!
    set points 0
    set last-opponents-move "cooperate"
    set grimed false
    update-color
  ]
...
to interact            ;; patch procedure
  select-move          ;; decide if to cooperate or not
  play                 ;; play your move against a random neighbour
end

to select-move
  if strategy = "random" [ ; 50% probability
    set move one-of ["cooperate" "defect"]
  ]
  ...
  if strategy = "tit for tat" [ ; repeat opponent's last choice
    set move last-opponents-move
  ]
end

to play
  ifelse (move = "cooperate") [
    ifelse ([move = "cooperate"] of one-of neighbors)

      ;; coopertate|cooperate
      [set points (points + payoff-r)
        set last-opponents-move "cooperate"]

    ;; cooperate|defect
    [set points (points + payoff-s)
      set last-opponents-move "defect"
      set grimed true
  ...
  [ifelse ([move = "cooperate"] of one-of neighbors)
    ...
    ;; defect|cooperate
    [set points (points + payoff-t)
      set last-opponents-move "cooperate"]

     ;; defect|defect
    [set points (points + payoff-p)
      set last-opponents-move "defect"
      set grimed true
      ;set defects (defects + 1)
      ]
...
to select-strategy  ;; select the best strategy beyond your neighbours every 8 ticks
  if((ticks mod 8) = 0)[
  if(points <  [points] of max-one-of neighbors [points])[
  set strategy ( [strategy] of max-one-of neighbors [points])
  ]
  ]
end
...
"" ""
PENS
"random" 1.0 0 -13345367 true "" "plot count patches with [pcolor = blue]"
"cooperate" 1.0 0 -10899396 true "" "plot count patches with [pcolor = green]"
"pen-2" 1.0 0 -1184463 true "" "plot count patches with [pcolor = yellow]"
"pen-3" 1.0 0 -2674135 true "" "plot count patches with [pcolor = red]"
"pen-4" 1.0 0 -955883 true "" "plot count patches with [pcolor = orange]"
"tranquilizer" 1.0 0 -8630108 true "" "plot count patches with [pcolor = violet]"
...
Fight!
Fight!
["random" "always cooperate" "always defect" "grim trigger" "tit for tat"] ["random" "always cooperate"] ["random" "always defect"] ["random" "grim trigger"] ["random" "tit for tat" "always cooperate"] ["always defect" "always cooperate"] ["always defect" "grim trigger"] ["always defect" "tit for tat"] ["always cooperate" "grim trigger"] ["always cooperate" "tit for tat"] ["grim trigger" "tit for tat"]
4
