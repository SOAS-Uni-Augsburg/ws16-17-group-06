...
to setup-turtles
  ...
  ask turtles [
    ...
    [
      ifelse (count turtles with [immune = true] < (population-size - population-size * initial-zombie-percentage / 100) * immunity / 100 ) [
      ; make the turtle a zombie
      set immune true
      set infected false
      set color green
...
to go
  ...
  ask turtles with [infected = true] [
    ifelse any? turtles-here with [infected = false] and random 100 < resistance
    [
      die
      ]
    [
    ifelse any? turtles-here with [infected = false and immune = false] [
      attack-human
      set energy energy + energy_gained
    ]
    [
      set energy energy - energy_loss
      ]
    ]
  ]
  ...
  ask turtles with [infected = false] [
    ifelse random 100 > mortality[
      if any? turtles-here with [infected = false and random 100 < reproduction]
      [
      ifelse random 100 < immunity [
        hatch 1 [set infected false set immune true set energy energy_start]
        ]
      [
         hatch 1 [set infected false set immune false set energy energy_start]
        ]
      ]
      ]
    [
...
to check-death
  ask turtles with [infected = true and immune = false] [
